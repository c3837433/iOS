...
-(void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info
{
    ...
    ALAssetsLibraryWriteImageCompletionBlock completeBlock = ^(NSURL *assetURL, NSError *error){
        if (!error) {
          urlString = [NSString stringWithFormat:@"%@",assetURL];
            NSLog(@"%@", urlString);
        }
    };
    ...
    // SAVE TO THE ASSETS LIBRARY
    ALAssetsLibrary* deviceLibrary = [ALAssetsLibrary new];
    [deviceLibrary writeImageToSavedPhotosAlbum:[takenImage CGImage] metadata:nil completionBlock:^(NSURL *assetURL, NSError *error)
    {
        if (error) {
            NSLog(@"error = %@", error);
        } else if (!error)
    ...
    // Try again
    /*
    [deviceLibrary writeImageToSavedPhotosAlbum:[takenImage CGImage] orientation:(ALAssetOrientation)[takenImage imageOrientation] completionBlock:^(NSURL *assetURL, NSError *error)
    {
        if (error) {
            NSLog(@"error");
        } else {
...
-(CIImage *)previousPhoto:(CIImage *)image lightLevel:(float)lightLevel shadowLevel:(float)shadowLevel vibranceLevel:(float)vibranceLevel
{
    ...
    CIFilter* vibrance = [CIFilter filterWithName:@"CIVibrance"
                                    keysAndValues:kCIInputImageKey,highlight.outputImage,@"inputAmount",[NSNumber numberWithFloat:vibranceLevel], nil];
